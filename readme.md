...
Either upload a file, provide a link to a .pdb file, or input a PDB ID from the [RCSB website](https://www.rcsb.org/) to change the biomolecule. Note that this tool only supports legacy .pbd files, which some entries from the RCSB site do not have. Biomolecules can also be located away from the viewport, so you may have to look around to find them.
